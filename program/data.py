...
class Bid:
    ...
    @classmethod
    def parse_line(cls, line, max):
        splited_line = re.split('\s+', line)
        identifier = int(splited_line[0])
        price = float(splited_line[1])
        goods_list = [i for i in map(lambda x: int(x), splited_line[2:-2]) if i < max]
...
class CAInstance:
    def __init__(self, goods_count, dummies_count, bids_count, bids_list):
    ...
    def from_file(cls, file_name):
        ...
        for line in lines:
            if line[0] == 'g':
                ...
                bids_list.append(Bid.parse_line(line, goods))
        return CAInstance(goods, dummies, bids, bids_list)
    ...
    def evaluate_bin(self, binary_vector):
        used_goods = [False] * self.all_goods_count()
        total_price = 0.0
        for i, b in enumerate(binary_vector):
            if b:
                bid = self.bids_list[i]
                if bid.is_avalible(used_goods):
                    total_price += bid.price()
                    # updates used goods
                    for i in bid.goods_identifiers():
